...
#===================
#Config addjusted base on wm_tunner.ipyb
    model = Sequential()
    model.add(Conv2D(144, (3, 3), activation='relu', padding='same',input_shape=(waferSizeX, waferSizeY, 3)))  					# 卷積層 (輸入)
    model.add(Dropout(0.25))           							    # Dropout
    model.add(MaxPooling2D((2, 2)))                        			# 池化層
    model.add(Conv2D(256, (3, 3), padding='same', activation='relu'))  # 卷積層
    model.add(Dropout(0.35))                              			# Dropout 層
    model.add(MaxPooling2D((2, 2)))                             		# 池化層
    model.add(Flatten())                                    			# 展平層
    model.add(Dropout(0.3))                                   		# Dropout
    model.add(Dense(736, activation='relu'))                      	# 密集層
    model.add(Dropout(0.3))                                     	    # Dropout
...
#===================
    ...
    model.add(Conv2D(filters=96, input_shape=(waferSizeX,waferSizeY,3), kernel_size=(4,4),strides=(1,1), padding='valid')) #(48-4)/1 +1 =45 (45x45)*96=48600
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(3,3), strides=(2,2), padding='valid'))                                               #(45-3)/2 + 1 = (22x22) * 96 
           
    # 2nd Convolutional Layer
    model.add(Conv2D(filters=256, kernel_size=(4,4), strides=(1,1), padding='valid'))                                       #22x22 x256
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(3,3), strides=(2,2), padding='valid')) # Pooling
    ...
    model.add(Flatten())
    # 1st Dense Layer
    model.add(Dense(512, input_shape=(48*48*3,)))
    model.add(Activation('relu'))
...
#===================
def AlexNetOrg(waferSizeX, waferSizeY): #224x224
#===================
    model = Sequential()
    # 1st Convolutional Layer
    ...
    return model
#===================
def vgg16_keras(waferSizeX, waferSizeY):
#===================
    vgg16_model = VGG16(include_top=False,weights=None,input_shape=(waferSizeX,waferSizeY,3))
    model = Sequential()
...
#===================
    ...
    model = Sequential()
    model.add(Conv2D(input_shape=(waferSizeX,waferSizeY,3),filters=64,kernel_size=(3,3),padding="same", activation="relu"))
